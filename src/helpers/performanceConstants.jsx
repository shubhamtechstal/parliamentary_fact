// export const performace_chipList = [
//   'Lok Sabha Productivity',
//   'Lok Sabha Attandance',
//   'Lok Sabha Question',
//   'Lok Sabha Debates',
//   'Lok Sabha Private Member Bills',
//   'Lok Sabha Adjiurnment',
//   'Lok Sabha Walkout',
//   'Lok Sabha on the floor',
//   'Lok Sabha Interruption',
//   'Lok Sabha Quoram Bell',
//   'Lok Sabha Not Recorded',
//   'Govt Bill',
// ];
export const performace_chipList = [
  {
    title: 'Lok Sabha Productivity',
    routeUrl: 'lok-sabha-productivity',
  },
  {
    title: 'Lok Sabha Attandance',
    routeUrl: 'lok-sabha-attandance',
  },
  {
    title: 'Lok Sabha Question',
    routeUrl: 'lok-sabha-question',
  },
  {
    title: 'Lok Sabha Debates',
    routeUrl: 'lok-sabha-debates',
  },
  {
    title: 'Lok Sabha Private Member Bills',
    routeUrl: 'lok-sabha-private-member-bills',
  },
  {
    title: 'Lok Sabha Adjournment',
    routeUrl: 'lok-sabha-adjourned',
  },
  {
    title: 'Lok Sabha Walkout',
    routeUrl: 'lok-sabha-walkout',
  },
  {
    title: 'Lok Sabha In The Well',
    routeUrl: 'lok-sabha-in-the-well',
  },
  {
    title: 'Lok Sabha Interruptions',
    routeUrl: 'lok-sabha-interruptions',
  },
  {
    title: 'Lok Sabha Quoram Bell',
    routeUrl: 'lok-sabha-quoram-bell',
  },
  {
    title: 'Lok Sabha Not Recorded',
    routeUrl: 'lok-sabha-not-recorded',
  },
  // {
  //   title: 'Govt Bill',
  //   routeUrl: 'govt-bill',
  // },
];

export const productivity_schedule = [
  {
    name: 'SITTING SCHEDULED',
    value: '02:22 Hrs.',
  },
  {
    name: 'TIME LOST',
    value: '00:00 Hrs.',
  },
  {
    name: 'LUNCH TIME',
    value: '00:00 Hrs.',
  },
  {
    name: 'Productivity',
    valuee: '100',
    percentage: '.28%',
  },
  {
    name: 'LATE SITTING',
    value: '00:00 Hrs.',
  },
  {
    name: 'OVER TIME',
    value: '00:00 Hrs.',
  },
  {
    name: 'ACTUAL SITTING',
    value: '02:22 Hrs.',
  },
];
export const productivity_bottomCardsdata = [
  {
    title: 'INTERRUPTIONS',
    value: '16500',
    linkText: 'Know More >>',
    page_url:'/parliament-performance/lok-sabha-performance/lok-sabha-interruptions'
  },
  {
    title: 'ADJOURNED',
    value: '540',
    linkText: 'Know More >>',
    page_url:'/parliament-performance/lok-sabha-performance/lok-sabha-adjourned'
  },
  {
    title: 'WALK OUT',
    value: '79',
    linkText: 'Know More >>',
    page_url:'/parliament-performance/lok-sabha-performance/lok-sabha-walkout'
  },
  {
    title: 'IN THE WELL',
    value: '134',
    linkText: 'Know More >>',
    page_url:'/parliament-performance/lok-sabha-performance/lok-sabha-in-the-well'
  },
  {
    title: 'NOT RECORDED',
    value: '50',
    linkText: 'Know More >>',
    page_url:'/parliament-performance/lok-sabha-performance/lok-sabha-not-recorded'
  },
  {
    title: 'QUORAM BELL',
    value: 'TBA',
    linkText: 'Know More >>',
    page_url:'/parliament-performance/lok-sabha-performance/lok-sabha-quoram-bell'
  },
];
export const questionsListData = {
  title: 'Lok Sabha Questions',
  percentage: '100.28%',
  date: 'Till Now',
  progressTitle: 'QUESTIONS',
  progressSubTitle: 'OREL',
};
export const privateMemberBillListData = {
  title: 'Lok Sabha Private Member Bill',
  percentage: '90.28%',
  date: 'Till Now',
  progressTitle: 'Private Member Bill',
  progressSubTitle: 'OREL',
};
export const debateListData = {
  title: 'Debate in Lok Sabha',
  progressTitle: 'IN DEBATE',
  progressSubTitle: 'PERFORMANCE',
  date: 'Till Now',
  listData: [
    {
      title: 'GOVERNMENT BILL',
      value: '25',
    },
    {
      title: 'STATUTORY RESOLUTION AND GOVERNMENT BILL',
      value: '24.5',
    },
    {
      title: 'MATTERS UNDER RULE 377',
      value: '24.5',
    },
    {
      title: 'DISCUSSION UNDER RULE 193',
      value: '24.5',
    },
    {
      title: 'RULE 184',
      value: '24.5',
    },
    {
      title: 'HALF-AN-HOUR DISCUSSION',
      value: '24.5',
    },
    {
      title: 'CALLING ATTENTION',
      value: '24.5',
    },
    {
      title: 'MOTION OF THANKS ON THE PRESIDENT ADDRESS',
      value: '24.5',
    },
    {
      title: 'BUDGET DISCUSSION',
      value: '24.5',
    },
    {
      title: 'PRIVATE MEMBER BILL',
      value: '24.5',
    },
    {
      title: 'PRIVATE MEMBER RESOLUTION',
      value: '24.5',
    },
    {
      title: 'BUSINESS OF THE HOUSE',
      value: '24.5',
    },
    {
      title: 'SUBMISSION BY MEMBERS',
      value: '24.5',
    },
    {
      title: 'ASSOCIATE BY SUBMISSION BY MEMBERS',
      value: '24.5',
    },
    {
      title: 'SPECIAL MENTION-ZERO HOURS',
      value: '24.5',
    },
    {
      title: 'ASSOCIATION IN SPECIAL MENTION',
      value: '24.5',
    },
    {
      title: 'STATEMENT BY MINISTER',
      value: '24.5',
    },
    {
      title: 'NO CONFIDENCE MOTION',
      value: '24.5',
    },
    {
      title: 'ADJOURNMENT MOTION',
      value: '24.5',
    },
    {
      title: 'GOVERNMENT RESOLUTION',
      value: '24.5',
    },
  ],
  listData2: [
    {
      title: 'Higest Questions Day',
      value: '24.56',
      date: '15AUG 2024',
    },
    {
      title: 'Higest Questions Session',
      value: 'WINTTER',
      date: '2024',
    },
    {
      title: 'Higest Written Questions Day by MPs',
      value: '400',
      date: '5AUG 2024',
    },
    {
      title: 'Lowest Questions Day',
      value: '24.56',
      date: '15AUG 2024',
    },
    {
      title: 'Lowest Questions Session',
      value: 'BUDGET',
      date: '2024',
    },
    {
      title: 'Lowest Written Questions Day by MPs',
      value: '230',
      date: '5AUG 2024',
    },
    {
      title: 'Higest Total Questions ask by Ministry',
      value: 'MINISTRY OF DEFANCE',
      date: '5AUG 2024',
    },
    {
      title: 'Higest Questions Topics',
      value: '400',
      date: 'Price Hike of Temotos', // New field for additional info
    },
    {
      title: 'Higest Total Questions ask by Ministry',
      value: 'MINISTRY OF DEFANCE',
      date: '5AUG 2024',
    },
    {
      title: 'Higest Total Questions ask by Ministry',
      value: '10',
      date: 'Price Hike of Temotos', // New field for additional info
    },
  ],
};

export const MpFundsectionData = {
  title: 'MP Fund-Local Development Fund (MPLADS)',
  readBeforeCheck: 'Read before check performance',
  fundAllocated: '2715 CR',
  percentage: '100.28%',
  date: 'Till Now',
  progressTitle: 'LOCAL',
  progressSubTitle: 'DEVELOPMENT FUND SPENT (MP FUND)',
  // localDevelopmentFund: {
  //   title: 'LOCAL DEVELOPMENT FUND SPENT (MP FUND)',
  //   range: '0 LOCAL DEVELOPMENT FUND 100+',
  // },
  fundData: [
    {
      title: 'FUND ALLOCATED',
      value: '27 CR',
    },
    {
      title: 'FUND RECOMMENDATIONS',
      value: '00',
      unit: '00000 CR',
    },
    {
      title: 'FUND SECTION',
      value: '00',
      unit: '00000 CR',
    },
    {
      title: 'FUND EXPENDITURE',
      value: '00',
      unit: '00000 CR',
    },
    {
      title: 'UN SPENT FUND',
      value: '00',
      unit: '00000 CR',
    },
    {
      title: 'REMAINING FUND HAVE FUND NOW',
      value: '00',
      unit: '00000 CR',
    },
  ],
  fundSpentInTopArea: {
    title: 'FUND SPENT IN TOP ARIA',
    items: [
      {
        value: '24.56',
        category: 'EDUCTAION',
      },
      {
        value: '24.56',
        category: 'EDUCTAION',
      },
      {
        value: '24.56',
        category: 'EDUCTAION',
      },
      {
        value: '24.56',
        category: 'LOCAL INFRASTRUCTURE',
      },
      {
        value: '24.56',
        category: 'EDUCTAION',
      },
      {
        value: '24.56',
        category: 'EDUCTAION',
      },
      {
        value: '24.56',
        category: 'EDUCTAION',
      },
      {
        value: '24.56',
        category: 'LOCAL INFRASTRUCTURE',
      },
      {
        value: '24.56',
        category: 'EDUCTAION',
      },
      {
        value: '24.56',
        category: 'EDUCTAION',
      },
      {
        value: '24.56',
        category: 'EDUCTAION',
      },
      {
        value: '24.56',
        category: 'LOCAL INFRASTRUCTURE',
      },
    ],
  },
  footer: 'MPs Parlicipation in Lok Sabha Attendance',
};
export const questionsDetailsData = {
  title: 'QUESTION IN LOK SABHA',
  filter: 'Filter',
  select_year_label: 'Select Year',
  year_options: [
    {
      label: '1st year',
      checked: true,
    },
    {
      label: '2nd year',
      checked: false,
    },
    {
      label: '3rd year',
      checked: false,
    },
    {
      label: '4th year',
      checked: false,
    },
    {
      label: '5th year',
      checked: false,
    },
  ],
  select_session_1_label: 'Select Session',
  session_1_options: [
    {
      label: 'Special Session 2019',
      checked: true,
    },
    {
      label: 'Monsoon Session 2019',
      checked: false,
    },
    {
      label: 'Winter Session 2019',
      checked: false,
    },
    {
      label: 'Budget Session 2020',
      checked: false,
    },
  ],
  select_session_2_label: 'Select Session',
  session_2_options: [
    {
      label: 'Special Session 2019',
      checked: false,
    },
    {
      label: 'Monsoon Session 2019',
      checked: false,
    },
    {
      label: 'Winter Session 2019',
      checked: false,
    },
    {
      label: 'Budget Session 2020',
      checked: false,
    },
  ],
  done_button: 'Done',
  questions: [
    {
      date: '25 NOV 2024',
      number: 'QUESTION NUM.1',
      subject:
        'LOCAL INFRASTRUCTURE , SUCH AS ROADS AND WATER / WASTEWATER NETWORKS , FORM',
      from_ministry_label: 'FROM MINISTERY',
      ministry: 'MINISTRY OF RURAL DEVELOPMENT',
      answer_by_label: 'ANSWER BY',
      answer_by: 'SHIVRAJ SINGH CHOUHAN',
      by_mps_label: 'BY MPS',
      mps: ['SHREE RAHUL GANDHI', 'SHREE KAMLESH PASWAN', 'SMT . SUPRIYA SULE'],
      content: [
        'Local infrastructure , such as roads and water / wastewater networks , form the backbone of the local economy . They enable the movement of people and goods , provide essential drinking water .',
        'Local infrastructure , such as roads and water / wastewater networks , form the backbone of the local economy . They enable the movement of people and goods , provide',
        'Local Infrastructure , such as roads and water / wastewater networks , form the backbone of the local economy . They enable the movement of people and goods , provide essential drinking water',
      ],
    },
    {
      number: 'QUESTION NUM.2',
      subject:
        'LOCAL INFRASTRUCTURE , SUCH AS ROADS AND WATER / WASTEWATER NETWORKS , FORM',
      from_ministry_label: 'FROM MINISTERY',
      ministry: 'MINISTRY OF RURAL DEVELOPMENT',
      answer_by_label: 'ANSWER BY',
      answer_by: 'SHIVRAJ SINGH CHOUHAN',
      by_mps_label: 'BY MPS',
      mps: ['SHREE RAHUL GANDHI', 'SHREE KAMLESH PASWAN', 'SMT SUPRIVA SULE'],
      content: [
        'Local infrastructure , such as roads and water / wastewater networks , form the backbone of the local economy . They enable the movement of people and goods , provide essential drinking water .',
        'Local infrastructure , such as roads and water / wastewater networks , form the backbone of the local economy . They enable the movement of people and goods , provide',
      ],
    },
    {
      // "date": "25 NOV 2024",
      number: 'QUESTION NUM.3',
      subject:
        'LOCAL INFRASTRUCTURE , SUCH AS ROADS AND WATER / WASTEWATER NETWORKS , FORM',
      from_ministry_label: 'FROM MINISTERY',
      ministry: 'MINISTRY OF RURAL DEVELOPMENT',
      answer_by_label: 'ANSWER BY',
      answer_by: 'SHIVRAJ SINGH CHOUHAN',
      by_mps_label: 'BY MPS',
      mps: ['SHREE RAHUL GANDHI', 'SHREE KAMLESH PASWAN', 'SMT . SUPRIYA SULE'],
      content: [
        'Local infrastructure , such as roads and water / wastewater networks , form the backbone of the local economy . They enable the movement of people and goods , provide essential drinking water .',
        'Local infrastructure , such as roads and water / wastewater networks , form the backbone of the local economy . They enable the movement of people and goods , provide',
        'Local Infrastructure , such as roads and water / wastewater networks , form the backbone of the local economy . They enable the movement of people and goods , provide essential drinking water',
      ],
    },
    {
      number: 'QUESTION NUM.4',
      subject:
        'LOCAL INFRASTRUCTURE , SUCH AS ROADS AND WATER / WASTEWATER NETWORKS , FORM',
      from_ministry_label: 'FROM MINISTERY',
      ministry: 'MINISTRY OF RURAL DEVELOPMENT',
      answer_by_label: 'ANSWER BY',
      answer_by: 'SHIVRAJ SINGH CHOUHAN',
      by_mps_label: 'BY MPS',
      mps: ['SHREE RAHUL GANDHI', 'SHREE KAMLESH PASWAN', 'SMT SUPRIVA SULE'],
      content: [
        'Local infrastructure , such as roads and water / wastewater networks , form the backbone of the local economy . They enable the movement of people and goods , provide essential drinking water .',
        'Local infrastructure , such as roads and water / wastewater networks , form the backbone of the local economy . They enable the movement of people and goods , provide',
      ],
    },
  ],
};
export const mpsDataNetionalRank = (mps__data =[]) => {
  const result = [];
  mps__data?.forEach((data) => {
    result.push({
      rank: data.national_rank,
      mp_id: data.mp_id,
      name: data.name,
      constituency: data.constituency,
      state_name: data.state_name,
      state_shortName: data.state_short_name,
      performance: data.national_percentage,
      partyName: data.party_short_name,
      participation: data.participation,
      rankTitle: 'National Rank:',
      image: data.image,
    });
  });
  return result;
};
export const mpsDataStateRank = (mps__data) => {
  const result = [];
  mps__data?.forEach((data) => {
    result.push({
      rank: data.state_rank,
      mp_id: data.mp_id,
      name: data.name,
      constituency: data.constituency,
      state_name: data.state_name,
      state_shortName: data.state_short_name,
      performance: data.state_percentage,
      participation: data.participation,
      partyName: data.party_short_name,
      rankTitle: 'State Rank:',
      image: data.image,
    });
  });
  return result;
};

export const attendance_details = [
  {
    title: 'Highest Attendance Day',
    date: '2024-12-20',
    count: 453,
  },
  {
    title: 'Highest Attendance Session',
    count: 'Winter Session 2024',
  },
  {
    title: 'Lowest Attendance Day',
    date: '2024-12-20',
    count: 453,
  },
  {
    title: 'Lowest Attendance Session',
    count: 'Winter Session 2024',
  },
];

// export const mpsDataNetionalRank = [
//   {
//     rank: '001',
//     name: 'Amit Kumar Singh',
//     constituency: 'Varanasi',
//     performance: '78.2',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '002',
//     name: 'Priya Reddy',
//     constituency: 'Hyderabad',
//     performance: '84.1',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '003',
//     name: 'Rajesh Mehta',
//     constituency: 'Ahmedabad East',
//     performance: '67.5',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '004',
//     name: 'Neha Bhandari',
//     constituency: 'Jaipur',
//     performance: '74.3',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '005',
//     name: 'Anil Yadav',
//     constituency: 'Lucknow',
//     performance: '69.8',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '006',
//     name: 'Simran Dey',
//     constituency: 'Kolkata North',
//     performance: '81.6',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '007',
//     name: 'Ravi Shankar',
//     constituency: 'Bhopal',
//     performance: '76.9',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '008',
//     name: 'Meena Patil',
//     constituency: 'Pune',
//     performance: '68.4',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '009',
//     name: 'Vikram Chaudhary',
//     constituency: 'Chandigarh',
//     performance: '72.1',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '010',
//     name: 'Sonal Verma',
//     constituency: 'Delhi South',
//     performance: '79.2',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '011',
//     name: 'Deepak Thakur',
//     constituency: 'Patna Central',
//     performance: '70.5',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '012',
//     name: 'Alok Jain',
//     constituency: 'Indore',
//     performance: '73.4',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '013',
//     name: 'Kavita Nair',
//     constituency: 'Bangalore Central',
//     performance: '77.8',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '014',
//     name: 'Manoj Deshmukh',
//     constituency: 'Nagpur',
//     performance: '80.1',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '015',
//     name: 'Ritika Joshi',
//     constituency: 'Dehradun',
//     performance: '65.9',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '016',
//     name: 'Shubham Bansal',
//     constituency: 'Agra',
//     performance: '74.0',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '017',
//     name: 'Anjali Das',
//     constituency: 'Guwahati',
//     performance: '78.6',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '018',
//     name: 'Nikhil Sharma',
//     constituency: 'Noida',
//     performance: '82.3',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '019',
//     name: 'Tanvi Rana',
//     constituency: 'Faridabad',
//     performance: '69.4',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '020',
//     name: 'Harsh Vardhan',
//     constituency: 'Amritsar',
//     performance: '75.5',
//     rankTitle: 'National Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
// ];
// export const mpsDataStateRank = [
//   {
//     rank: '001',
//     name: 'Amit Kumar Singh',
//     constituency: 'Varanasi',
//     performance: '78.2',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '002',
//     name: 'Priya Reddy',
//     constituency: 'Hyderabad',
//     performance: '84.1',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '003',
//     name: 'Rajesh Mehta',
//     constituency: 'Ahmedabad East',
//     performance: '67.5',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '004',
//     name: 'Neha Bhandari',
//     constituency: 'Jaipur',
//     performance: '74.3',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '005',
//     name: 'Anil Yadav',
//     constituency: 'Lucknow',
//     performance: '69.8',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '006',
//     name: 'Simran Dey',
//     constituency: 'Kolkata North',
//     performance: '81.6',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '007',
//     name: 'Ravi Shankar',
//     constituency: 'Bhopal',
//     performance: '76.9',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '008',
//     name: 'Meena Patil',
//     constituency: 'Pune',
//     performance: '68.4',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '009',
//     name: 'Vikram Chaudhary',
//     constituency: 'Chandigarh',
//     performance: '72.1',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '010',
//     name: 'Sonal Verma',
//     constituency: 'Delhi South',
//     performance: '79.2',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '011',
//     name: 'Deepak Thakur',
//     constituency: 'Patna Central',
//     performance: '70.5',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '012',
//     name: 'Alok Jain',
//     constituency: 'Indore',
//     performance: '73.4',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '013',
//     name: 'Kavita Nair',
//     constituency: 'Bangalore Central',
//     performance: '77.8',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '014',
//     name: 'Manoj Deshmukh',
//     constituency: 'Nagpur',
//     performance: '80.1',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '015',
//     name: 'Ritika Joshi',
//     constituency: 'Dehradun',
//     performance: '65.9',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '016',
//     name: 'Shubham Bansal',
//     constituency: 'Agra',
//     performance: '74.0',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '017',
//     name: 'Anjali Das',
//     constituency: 'Guwahati',
//     performance: '78.6',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '018',
//     name: 'Nikhil Sharma',
//     constituency: 'Noida',
//     performance: '82.3',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '019',
//     name: 'Tanvi Rana',
//     constituency: 'Faridabad',
//     performance: '69.4',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
//   {
//     rank: '020',
//     name: 'Harsh Vardhan',
//     constituency: 'Amritsar',
//     performance: '75.5',
//     rankTitle: 'State Rank:',
//     image: 'https://via.placeholder.com/50',
//   },
// ];
const agendaData = [
  {
    date: '25 NOV 2024',
    items: [
      {
        adjournment: '1',
        time: '11:15 AM',
        subject:
          'Local infrastructure, such as roads and water/wastewater networks, form Local Infrastructure, such as roads and water/wastewater networks, form Local infrastructure, such as roads and water/wastewater networks, form',
      },
      {
        adjournment: '2',
        time: '11:15 AM',
        subject:
          'Local infrastructure, such as roads and water/wastewater networks, form Local Infrastructure, such as roads and water/wastewater networks, form Local infrastructure, such as roads and water/wastewater networks, form',
      },
      {
        adjournment: '3',
        time: '11:15 AM',
        subject:
          'Local infrastructure, such as roads and water/wastewater networks, form Local Infrastructure, such as roads and water/wastewater networks, form Local infrastructure, such as roads and water/wastewater networks, form',
      },
    ],
  },
  {
    date: '26 NOV 2024',
    items: [
      {
        adjournment: '1',
        time: '11:15 AM',
        subject:
          'Local infrastructure, such as roads and water/wastewater networks, form Local Infrastructure, such as roads and water/wastewater networks, form Local infrastructure, such as roads and water/wastewater networks, form',
      },
    ],
  },
];