...
export const fetchMpsPerformanceData = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const url = `${BASE_URL}/mps_performance_data.php?${params.toString()}`;
...
export const fetchPopulerMpsData = createAsyncThunk(
  ...
  async (param, { rejectWithValue }) => {
    try {
      const url = `${BASE_URL}/popular_mps.php`;
      const response = await fetch(url);
      if (!response.ok)
        throw new Error(`HTTP error! Status: ${response.status}`);
...
export const fetchConstituencyPopulerMps = createAsyncThunk(
  ...
  async (param, { rejectWithValue }) => {
    try {
      const url = `${BASE_URL}/popular_mps_constituency_performance.php`;
      const response = await fetch(url);
      if (!response.ok)
        throw new Error(`HTTP error! Status: ${response.status}`);
